						...
						<template  #cell(Actions)="data">
							...
							<b-dropdown variant="link" no-caret :right="$store.state.appConfig.isRTL">
								...
								<template #button-content>
									<img src="../comex/more-vertical.svg" size="16" class="align-middle text-body" />
								</template>
								

								<b-dropdown-item v-if="(role!='ENDUSER' && role!='OPUSER')" @click="deleteBill(data.item.id)">
									<img src="../comex/trash.svg" size="16" class="align-middle text-body" />
...
export default {
	...
	computed: {
		...mapState({
			...
			OpUserDetails: (state) => {
				return state.Opstore.OpUserDetails
			},
			role: (state) => {
